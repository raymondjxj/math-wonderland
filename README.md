...
| 板块 | 内容 |
|---|---|
| 🪐 年级课程 | 二~六年级共 67 个单元（对齐人教版目录），标准课 7 幕、概念密集课 8 幕探究模板 |
| 🎬 分镜动画 | 故事幕支持多镜头 SVG 动画：镜头推拉、元素高亮、字幕、可选语音解说（默认关） |
...
| 🗺️ 知识地图 | 四大领域 × 子链 × 前置依赖：看清每课从哪里来、到哪里去 |
| ⚡ 口算/笔算天天练 | 按年级自动出题，笔算题答完自动展示竖式；错题自动收录 |
| 📒 错题本 | 间隔重复排期（当场→1天→3天→7天毕业），前置地基诊断 |
| 📋 学习报告 | 能力雷达、年级进度、错题分析、下一步建议，本地生成可打印 |
| 📦 PWA 离线 | 全站可装到桌面，二次访问零网络可用 |
